...
type Event struct {
	...
	IsSyscall bool              // true if this is a syscall event or false of any sub event
	Args      map[string]string // argv if this is a syscall, otherwise just the event's data map
}
...
func parseEvent(data string) (Event, error) {
	...
	if m != nil && len(m) == 4 {
		...
		for len(args) > 0 {
			...
			if args[0] == '"' {
				if argValue, offset = parseNext(args[1:], '"'); offset == -1 {
					break
				}
				args = args[offset+3:]
			} else {
				if argValue, offset = parseNext(args, ' '); offset == -1 {
					break
				}
				args = args[offset+1:]
			}
